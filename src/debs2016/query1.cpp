...
// -----------------------------------------------------------------------------
std::shared_ptr<Topology> buildQuery1(PFabricContext& ctx,
                                      boost::filesystem::path& dataPath) {
  auto endTime =
      TimestampHelper::stringToTimestamp("2016-12-31T23:59:59.000+0000");
  auto t = ctx.createTopology();
  auto ttl = ctx.createStream<TTLType>("ttl");
  auto postTable = ctx.getTable<CommentedPostType::element_type, long>("Posts");
  ...
  auto posts =
      t->newStreamFromFile(postPath.string())
          .extract<RawPostType>('|')
  ...
  auto maxTime =
      posts
          ...
          .notify<TimestampTupleType>(
              [&](auto tp, bool outdated) { globalTime.set(get<0>(tp)); },
              [&](auto pp) {
                // in case of end of stream we set the clock to a time in the
                // future
  ...
  auto postsToTable =
      posts
          .map<PostType, CommentedPostType>(
              [](auto tp, bool) -> CommentedPostType {
                return makeTuplePtr(get<0>(tp), get<1>(tp), get<2>(tp),
                                    10,  // initial score = 10
                                    makeCommentorList());
              })
          ...
          .map<CommentedPostType, TTLType>([](auto tp, bool) -> TTLType {
            ...
            return makeTuplePtr(get<1>(tp), get<0>(tp) + 1000l * 60 * 60 * 24,
                                10);
          })
  ...
  auto comments =
      t->newStreamFromFile(commentPath.string())
          .extract<RawCommentType>('|')
          .map<RawCommentType, CommentType>([](auto tp, bool) -> CommentType {
            auto res =
                makeTuplePtr(TimestampHelper::stringToTimestamp(get<0>(tp)),
                             get<1>(tp), get<5>(tp), get<6>(tp));
            // handle null values
          ...
          .statefulMap<CommentType, CommentType, Comments2PostMap>(
              [](auto tp, bool, auto state) -> CommentType {
                auto postID = tp->isNull(3)
                                  ? state->findPostIdForComment(get<2>(tp))
                                  : get<3>(tp);
                // std::cout << "-> " << tp << " ==> post: " << postID <<
                // std::endl;
                state->registerPostForComment(get<1>(tp), postID);
                return makeTuplePtr(get<0>(tp), get<1>(tp), get<2>(tp), postID);
              })
          // make sure that comments are not newer than the posts we have
          // already processed
          .barrier<CommentType>(
              globalTime.mCondVar, globalTime.mCondMtx,
              [&](auto tp) -> bool { return get<0>(tp) < globalTime.get(); })
          .assignTimestamps<CommentType, 0>()
          // comments are outdated after 10 days
          .slidingWindow<CommentType>(WindowParams::RangeWindow,
                                      60 * 60 * 24 * 10)
          .keyBy<CommentType, 3, long>()
          // update the table by adding or removing the comment to its post
          .updateTable<CommentType, CommentedPostType, long>(
              postTable,
              [](auto tp, bool outdated, auto oldRec) -> bool {
                auto tup =
                    makeTuplePtr(get<0>(oldRec), get<1>(oldRec), get<2>(oldRec),
                                 get<3>(oldRec),
                                 outdated ? removeCommentor(get<4>(oldRec), tp)
          ...
          .map<CommentType, TTLType>([](auto tp, bool) -> TTLType {
            return makeTuplePtr(get<3>(tp), get<0>(tp) + 1000l * 60 * 60 * 24,
                                10);
          })
          .toStream<TTLType>(ttl);
  ...
  auto scoreUpdates =
      t->fromStream<TTLType>(ttl)
          // ttl tuples are blocked until they are on time
          .barrier<TTLType>(globalTime.mCondVar, globalTime.mCondMtx,
                            [&](auto tp) -> bool {
                              //   std::cout <<
                              //   TimestampHelper::timestampToString(get<1>(tp))
                              //   << " -- " <<
                              //   TimestampHelper::timestampToString(globalTime.get())
                              //   << std::endl;
          ...
          // update the score of the post in the table - the key is taken fom
          // the ttl tuple
          .updateTable<TTLType, CommentedPostType, long>(
              postTable,
              [](auto tp, bool outdated, auto oldRec) -> bool {
                auto tup = StreamElementTraits<CommentedPostType>::create(oldRec); // CommentedPostType
                auto score = updateScore(tup, globalTime.get());
          ...
          .map<TTLType, TTLType>([](auto tp, bool) -> TTLType {
            return makeTuplePtr(get<0>(tp), get<1>(tp) + 1000l * 60 * 60 * 24,
                                get<2>(tp) - 1);
          })
  ...
  //
  auto topk =
      t->newStreamFromTable<CommentedPostType, long>(postTable)
          // TODO: maintain top-3
...
int main(int argc, char** argv) {
  ...
  auto postTable =
      ctx.createTable<CommentedPostType::element_type, long>("Posts");
