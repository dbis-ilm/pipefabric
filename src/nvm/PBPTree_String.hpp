#ifndef PBPTree_String_hpp_
#define PBPTree_String_hpp_
...
//const std::string LAYOUT = "PBPTree_String";
...
//        other.branch = tempBranch;
 ...
 public:
   ...
   */
  typedef std::function<void(const KeyType &key, const std::string &val)>
      ScanFunc;

  /**
   * Constructor for creating a new B+ tree.
   */
  PBPTree_String() : depth(0) {
   ...
   */
  void insert(const KeyType &key, const std::string &val) {
   ...
   */
  bool lookup(const KeyType &key, std::string *val) const {
    assert(val != nullptr);
    bool result = false;
    ...
    if (pos < leafNode->numKeys && leafNode->keys->at(pos) == key) {
      // we found it!
      *val = leafNode->values->at(pos).data();
      result = true;
    }
    return result;
  }
  ...
  bool insertInLeafNode(persistent_ptr<LeafNode> node, const KeyType &key,
                        const std::string &val, SplitInfo *splitInfo) {
    bool split = false;
    auto pos = lookupPositionInLeafNode(node, key);
    if (pos < node->numKeys && node->keys->at(pos) == key) {
      // handle insert of duplicates
      node->values->at(pos).set(const_cast<std::string*>(&val));
      return false;
    }
    if (node->numKeys == M) {
  ...
  void insertInLeafNodeAtPosition(persistent_ptr<LeafNode> node, unsigned int pos,
                                  const KeyType &key, const std::string &val) {
    assert(pos < M);
    assert(pos <= node->numKeys);
  ...
  struct LeafNode {
    ...
    LeafNode() : numKeys(0), nextLeaf(nullptr), prevLeaf(nullptr) {
      ...
      transaction::exec_tx(pop, [&] {
        ...
        values = make_persistent<std::array<PString, M>>();
      });
    }
   ...
   //    std::endl; }
    ...
    persistent_ptr<std::array<PString, M>> values;   //< the actual values
